...
library (data.table)
    ...
    for (j in 1:length(sample[,1])){
      ...
      for (k in 1:200){
        ...
        dist.y<-diff(trip[,2])
        slope<-dist.x/dist.y
        qtl.slope<-quantile(dist.x,c(0.1,0.25,0.5,0.75,0.9)) 
        ...
        Row<-c(time,driven.time,qtl.slope,qtl.speed,qtl.accel)
        if (k==1 & j==1){
  ...
  colnames(data)<-c("prob","time","driven.time"
                    ,paste("qtl.slope",c(0.1,0.25,0.5,0.75,0.9),sep=".")
...
prep.results<-function(init=1,end=5){
  ...
  for (i in init:end){
    ...
    a.pca<-princomp(scaled.data,cor=TRUE)
    ...
    b.pca<-kmeans(a.pca$scores,centers=2,nstart=10)
